	...
	$.ajax ({
		...
		success : function(json) {// 如果调用成功
			...
			if(json.data.count === 0) {
				...
				message = message + '<span class="am-icon-envelope-o"></span> 消息'
						+ '<span class="am-badge am-badge-warning">' + json.data.count + '</span>'
			}
...
function getMessage(json) {
	...
	$.each(json.message, function(idx, f) {
		if(json.sendUser===f.messageSend && json.acceptUser===f.messageAccept) {
			chatText = chatText + "<li class='content_li'><img class='chat_img content_li_img content_li_imgleft' src='http://s.amazeui.org/media/i/demos/bing-1.jpg' alt=''>"
			...
			chatText = chatText + "<li class='content_li'><img class='chat_img content_li_img content_li_imgright' src='http://s.amazeui.org/media/i/demos/bing-1.jpg' alt=''>"
